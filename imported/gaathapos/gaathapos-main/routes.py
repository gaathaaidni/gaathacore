...
@dashboard_api_bp.route('/profitability')
@login_required
@admin_required
def menu_profitability():
    """
    Calculates and returns the profitability of each menu item.
    """
    restaurant_id = current_user.restaurant_id
...
@dashboard_api_bp.route('/inventory-items')
@login_required
@admin_required
def get_inventory_items():
    """
    Returns a list of all inventory items for the current restaurant.
    Used to populate the recipe management modal.
    """
    restaurant_id = current_user.restaurant_id
    items = InventoryItem.query.filter_by(restaurant_id=restaurant_id).order_by(InventoryItem.name).all()
    return jsonify([{
        'id': item.id,
        'name': item.name,
        'unit': item.unit
